...
Symbolisen konekielen käskyjen lisäksi koodissa voi olla ohjauskäskyjä *symbolisen konekielen kääntäjälle*, joka voi vielä välittää jotkut ohjauskäskyt *lataajalle*. Tällaisia käskyjä sanotaan *pseudokäskyiksi* tai *valekäskyiksi*, koska ne näyttävät koenkäskyiltä, mutta eivät ole suoritettavaa koodia. Ttk-91:ssä on vain kaksi valekäskyä. Ohjauskäskyllä DC (data constant) varataan tilaa muistista kokonaislukuarvoiselle muuttujalle tai vakiolle arvo ja annetaan sille alkuarvo. Vakio eroaa muuttujasta siinä, että sen arvoa ei muuteta koodissa. Ohjauskäskyllä DS (data segment) varataan vain tilaa yhden tai useamman sanan verran muistista. Molemmilla ohjauskäskyillä DC ja DS määritellään myös symbolit, joiden arvoiksi tulee varattujen muistialueiden osoite.

```
n   dc    50    ; ohjelmassa käytetyn taulukon koko, alkuarvo n
tbl ds    50    ; 50-alkioinen taulukko, alustamaton arvoiltaan 
x   dc    0     ; muuttuja x, alkuarvo 0
```

Jos edellämainitut valekäskyt sijaitsevat annetussa järjestyksessä missä päin tahansa ohjelmakoodia ja ohjelmakoodin pituus on 85 konekäskyä (osoitteissa 0-84), niin symbolin n arvoksi tulee 85, symbolin tbl arvoksi 86 ja symbolin x arvoksi 136. Vakio n on siis osoitteessa 86, taulukko tbl osoitteessa 86 ja muuttuja x osoitteessa 136.
...
```
ifdef _floats_       ; onko suoritinversiossa liukulukukäskyjä?
    fadd   f1, f2, f3      ; laske liukulukukäskyillä
else                 ; tee liukulukulaskenta keskeytyskäsittelijässä
...
### Symbolinen konekieli
Ohjelmakoodin esitystapa, jossa konekäskyjen useat kentät voidaan esittää symbolien avulla. Esimerkiksi käskykoodi voi olla "add", eikä vain yhteenlaskukäskyn numeerinen käskykoodi 17. Samoin muuttujiin voidaan viitata niiden nimien (esim "x") avulla, eikä niiden muistisoitteiden avulla. Lisäksi konekäskyt kentät erottuvat toisistaan selkeästi, kun konekielessä kaikki kentät muodostavat vain yhden suuren kokonaisluvun.

### Konekieli
Ohjelman suorittimelle sopivat esitystapa, jossa kukin konekäsky on yksi iso kokonaisluku. Konekielinen esitysmuoto saadaan kääntämällä ohjelma joko sen symbolisesta konekielisestä esitysmuodosta tai korkean tason kielen esitysmuodosta.
