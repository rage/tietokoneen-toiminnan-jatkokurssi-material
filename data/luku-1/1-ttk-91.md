...
Jälkimmäisen operandin arvo (tai muistisoite) määritellään siis käyttäen kolmea käskyn kenttää: moodi M, indeksirekisteri Ri ja vakio-osa ADDR. 

Jos moodi on nolla (M=0), niin jälkimmäinen operandi on suoraan rekisterin Ri arvo lisättynä vakion arvolla, eli se on (Ri)+ADDR. 
Jos M=1, niin kyseessä on (aritmeettis-loogisten käskyjen yhteydessä) suora muistiosoitus ja jälkimmäinen operandi löytyy muistiosoitteesta (Ri)+ADDR, eli se on mem((Ri)+ADDR).
Jos M=2, niin kyseessä on (aritmeettis-loogisten käskyjen yhteydessä) epäsuora muistiosoitus ja jälkimmäisen operandin osoite löytyy muistiosoitteesta (Ri)+ADDR. Varsinainen jälkimmäinen operandi löytyy sitten tämän osoittamasta muistipaikasta, ja eli se on mem(mem((Ri)+ADDR)).
